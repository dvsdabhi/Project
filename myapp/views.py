...
def profile(request):
    ...
    if request.method == 'POST':
        if request.POST['password'] == request.POST['confirmpassword']:
            ...
            return render(request,'profile.html',{'msg':'Password And Confirmpassword Does Not Match','uid':uid})
...
def contact(request):
    if request.method == "POST":
        ...
        message = f"""Hello Admin! Someone is tring to contactr you HEre are the details as follow : 
                ...
                message :  {request.POST['message']}"""
        email_from = settings.EMAIL_HOST_USER
        recipient_list = ['email.dabhid962@gmail.com', ]
        send_mail( subject, message, email_from, recipient_list )
...
def view_product(request,pk):
    ...
    many = Product.objects.filter(category=cate)
    print(many)
    return render(request,'view-product.html',{"uid":uid,'product':product,'many':many})
...
def buy_order(request,pk):
    ...
    currency = 'INR'
    amount =  int(int(product.price) * int(quantity))*100  # Rs. 200
    ...
    context['price'] = int(product.price) * int(quantity)
    ...
    if request.method == "POST":
        # try:
            ...
            if result is None:
                ...
                amount = int(int(temp['product'].price)*int(temp['quantity']))*100 # Rs. 200
                ...
                try:
                    ...
                    temp['product'].quantity -= int(temp['quantity'])
                    temp['product'].save()
                    try:
                        del temp
                    except:
...
def new1(requset):
    return render(requset,'new1.html')
...
def seller_buy_product(request):
    ...
    if request.method == 'POST':
        ...
        if select == 'pending':
            ...
            select = True
    return render(request,'view-order.html',{'uid':uid,'buys':buys,'select':select})
